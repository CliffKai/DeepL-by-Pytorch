...
| 阶段 | 文件 | 数据集 | 可训练参数 | 超参数 |
|------|------|--------|-----------|--------|
| Stage 1: 预训练 | `train.py` | [LLaVA-CC3M-Pretrain-595K](https://huggingface.co/datasets/liuhaotian/LLaVA-CC3M-Pretrain-595K) (595K图文对) | linear1, linear2 (投影层) | bs=8, lr=1e-4, epoch=1, ga=8 |
| Stage 2: 单图SFT | `sft_train.py` | 图片: COCO train2014, 文本: [Chinese-LLaVA-Vision-Instructions](https://huggingface.co/datasets/LinkSoul/Chinese-LLaVA-Vision-Instructions) 中的 llava_instruct_230k.json (150K+80K合并, 237K多轮对话) | Qwen2.5 全参数 | bs=2, lr=5e-5, epoch=2, ga=8, warmup_ratio=0.03, cosine |
| Stage 3: 多图SFT | `sft_train_multi_images.py` | 多图对话数据集 (parquet格式) | Qwen2.5 全参数 | bs=4, lr=1e-5, epoch=1, ga=2, warmup_ratio=0.03, cosine |
