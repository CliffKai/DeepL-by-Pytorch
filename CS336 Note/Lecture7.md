...
>           * G1 **只存** $L_2$ 的 B,C,D...
>   * **计算过程：**
>     1.  **Forward/Backward:** 同上，大家各自算出局部梯度 E。
>     2.  **通信 (Reduce Scatter):** 同上，大家交换 E，每个人都拿到完整的总梯度。
>     3.  **Update (变化开始):**
>           * **G0:** "我是 $L_1$ 的管家，我有 $L_1$ 的 B,C,D。" -\> G0 更新 $L_1$ 的 A。
>           * **G1:** "我是 $L_2$ 的管家，我有 $L_2$ 的 B,C,D。" -\> G1 更新 $L_2$ 的 A。
...
> G0 既然只负责更新 $L_1$，那它为什么要拿 $L_2, L_3, L_4$ 的梯度 E 呢？没用啊！
> 
>   * **存储（显存里有什么）：**
>       * **A (权重):** 大家都有完整的（为了算梯度）。
>       * **B, C, D:** 切分了（同 Stage 1）。
>       * **E (梯度):** **也切分了！** G0 显存里只留 $L_1$ 的总梯度。
>   * **计算过程：**
>     1.  **Forward/Backward:** 同上，大家各自算出局部梯度 E。
>     2.  **通信 (Reduce-Scatter):** **这里变了！**
>           * G0 对其他人喊："把你们 $L_1$ 的梯度给我！其他的我不要！"
>           * G1 对其他人喊："把你们 $L_2$ 的梯度给我！"
>           * 通信结束后，G0 手里**只有 $L_1$ 的总梯度**。它没有 $L_2$ 的梯度（也不需要）。
>     3.  **Update:**
>           * G0 用 $L_1$ 的总梯度更新 $L_1$ 的 A。
>     4.  **通信 (All-Gather):** 同上，把更新好的 A 广播给大家。
>       * **结局：** **又节省了 3/4 的 E 显存。**
> 4. ZeRO Stage 3：切分参数 ($P_{os+g+p}$)
> 最极致的一步。G0 既然平时不更新 $L_2$，那存着 $L_2$ 的 A (权重) 干嘛？
>   * **存储（显存里有什么）：**
>       * **B, C, D, E:** 切分了（同 Stage 2）。
>       * **A (权重):** **也切分了！**
>           * G0 显存里**只有 $L_1$ 的 A**。
>           * G1 显存里**只有 $L_2$ 的 A**。
>   * **计算过程（变得很忙）：**
>     1.  **Forward:**
>           * 要算第一层了：大家都要用 $L_1$。G0 有，G0 Broadcast 给 G1, G2, G3。算完后，**G1, G2, G3 立刻把 $L_1$ 删掉**。
>           * 要算第二层了：大家都要用 $L_2$。G1 Broadcast 给其他人。算完后，**其他人立刻删掉**。
>     2.  **Backward:**
>           * 同理，算哪层，就找那层的管家临时要参数 A。用完即删。
>     3.  **Update:**
>           * G0 既然只有 $L_1$ 的全套数据（A, B, C, D, E），它就自己默默更新自己的 $L_1$。
>           * 更新完，它也不用广播给别人（因为别人本来就不存 $L_1$）。
>       * **结局：** 每个 GPU 只存了 1/4 的所有东西。显存占用极低，但**通信量（为了临时借参数 A）变得很大**。
